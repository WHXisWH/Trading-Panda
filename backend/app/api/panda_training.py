...
def _intent_dict(row: OrderIntent) -> dict:
    return {
        ...
        "max_slippage_bps": float(row.max_slippage_bps) if row.max_slippage_bps is not None else None,
        "final_score": float(row.final_score) if row.final_score is not None else None,
        "reason": row.reason,
        "decision_hash": row.decision_hash,
        "proof_eligible": row.proof_eligible,
        "proof_requested": row.proof_requested,
        "proof_request_source": row.proof_request_source,
