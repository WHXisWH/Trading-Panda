...
async def prepare_panda(panda_id: str, pair: str) -> int:
    database.ensure_engine()
    if database.AsyncSessionLocal is None:
        print("DATABASE_URL is not configured", file=sys.stderr)
        return 1

    parsed = build_e2e_buy_strategy(pair)
    async with database.AsyncSessionLocal() as session:
        ...
        strategy.raw_text = f"E2E fixture: buy {pair} when RSI is oversold"
...
if __name__ == "__main__":
    args = parse_args()
    raise SystemExit(asyncio.run(prepare_panda(args.panda_id, args.pair)))
