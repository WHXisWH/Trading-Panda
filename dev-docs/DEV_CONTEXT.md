...
| 2026-06-20 | **Review / Skill Memory 闭环修复**：补齐 SELL 平多复盘语义，breakeven 平仓也保留 entry/exit 证据；修复 `SkillMemoryWorker` 未 `await apply_skill_update_from_review`；清理重复 `publish_review` 定义；新增 post-commit hook 与 skill worker 单测。 | `TradeFactWriter` 平仓后可 enqueue review，review 完成后可进入 Skill Memory 更新链路；本地 `pytest tests/test_trade_fact_close.py tests/test_review_logic.py tests/test_skill_memory_worker.py tests/test_post_commit_hooks.py -q` 通过（14 passed），修改模块 `py_compile` 通过。 |
| 2026-06-20 | **本地 dev 环境启动**：创建本地 Docker `tp-postgres`(`127.0.0.1:55432`) 与 `tp-redis`(`127.0.0.1:6379`)；新增未提交本地 env：`backend/.env`、`market-monitor/.env`、`frontend/.env.local`；用 Python 3.11.5 创建 `backend/.venv-local`、`market-monitor/.venv-local` 并安装依赖/`socksio`；本地 DB 执行 Alembic head `007` 与 `seed_dev.py`。 | 本地 backend `:8000` health：DB connected、market consumer subscribed；market-monitor `:8001` health ok、Redis connected、DeepBook candles 可取；frontend `:3000` 首页/BFF market health/candles 200。WebSocket Hub 未启动，仍需 Upstash REST URL/token。 |
| 2026-06-20 | **本地 Upstash/WebSocket Hub 配置**：将本地 backend 与 market-monitor `REDIS_URL` 切到同一 Upstash `rediss://`；同步 backend/frontend/websocket `JWT_SECRET`；新增未提交 `websocket/.dev.vars`（Upstash REST URL/token）；用 Node 22 + `corepack pnpm` 启动 Wrangler dev。 | 四服务本地运行：backend `:8000` DB connected 且 `market_consumer.subscribed=true`/`message_count=1`；market-monitor `:8001` ok；frontend `:3000` 200；WebSocket Hub `localhost:8787/health` ok。 |
| 2026-06-20 | **Review / Skill Memory 本地 DB 闭环复验**：四服务健康后，复跑 Review/Skill 单元测试并用本地 DB 临时事务验证 closed Trade Fact → `position_closed` job → `trade_reviews` → `review_completed` job → `skill_memories` / `skill_versions`。同时 `walrus.upload_blob` 增加 datetime/date/Decimal JSON 序列化支持并补单测。 | `pytest tests/test_trade_fact_close.py tests/test_review_logic.py tests/test_skill_memory_worker.py tests/test_post_commit_hooks.py tests/test_walrus_integration.py -q` 通过（16 passed）；DB 事务验证生成 1 review + 1 skill memory + 1 skill version 后 rollback。Walrus publisher 当前返回 HTTP 404，归档 job 保持 pending/retry，不阻塞 Review/Skill。 |
| 2026-06-20 | **Training Ledger API 本地闭环验收**：为 seed Panda 补 dev-only active vault/policy，签发 JWT，经 `POST /panda/:id/simulation/start` 启动 actor；手动发布 fresh `market:tick:SUI-USDC` 验证 BUY 写入 OrderIntent/TradeFact；临时切 SELL 策略并连续发布 SELL tick 触发平仓复盘，随后恢复 seed BUY 策略并停止 simulation。 | 本地 DB 出现 7 OrderIntent、5 TradeFact、1 TradeReview、1 SkillMemory、1 SkillVersion；closed SELL TradeFact 保留 entry/exit 证据且 `review_status=reviewed`；market-monitor 已重启，`:8001/health` ok 且 Redis connected。 |
| 2026-06-20 | **本地钱包入口禁用 Stashed 默认展示**：`WalletProvider` 改为仅当 `NEXT_PUBLIC_ENABLE_STASHED_WALLET=true` 时注册 Stashed；`.env.example` 增加该开关注释。 | 避免本地连接钱包误点 Stashed 后卡在重定向/握手 loading；默认只展示真实浏览器扩展钱包。`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **钱包已连接后的 Sign In 按钮修复**：`WalletButton` 在已有 `account` 但未登录时，不再重新打开 Connect modal，而是直接调用 `loginWithWallet()` 触发 personal-message 签名登录。 | 连接扩展钱包后点击 `Sign In` 应弹出钱包签名确认；`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **Onboarding 问卷选项深色选中态修复**：抽出 `surveyOptionClass`，统一经验/风格/亏损/指标选项的 selected、hover、focus-visible 样式，移除浅色 `bg-primary-50` 与默认白底焦点观感。 | 选中项保持黑金绿 inset 质感，不再出现白色高亮条；`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **Onboarding 亏损步骤去除孤立 emoji**：第 3 步 max loss 删除居中 👍/😰/🐼 图标，改为随选择变化的风险说明卡（保守/平衡/进取/高风险）。 | 页面视觉不再出现突兀漂浮图标，风险含义更清楚；`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **Mint 钱包网络保护**：`useMintPanda` 在扩展钱包 `account.chains` 不包含本站 `NEXT_PUBLIC_SUI_NETWORK` 时阻止 mint，并在 Mint 页提示切换到 Sui Testnet。 | 避免钱包在 Mainnet 执行 Testnet package 调用时报底层 `Package object does not exist`；`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **Mint 后 Profile → Agent Wallet 浏览器集成验证**：通过 Chrome AppleScript 验证新 mint Panda 出现在 `/profile`，CTA 指向 `/agent-wallet?panda=3bba4b6c-0583-4485-9742-352c2e9c0253&step=no-vault`；本地 `backend/.env` 补 `AGENT_SIGNER_ADDRESS=0x4e94...41fa` 并重启 backend。 | Profile 显示 1 只 Panda、`NEEDS WALLET SETUP`；Agent Wallet 识别 NFT object `0xaaf9…d0b0`、signer configured，`Confirm training setup` 可进入 `Create PandaVault + Policy` 签名前弹窗。未代签钱包交易。 |
| 2026-06-20 | **Agent Wallet → Feed Strategy → Start Training 浏览器集成验证**：用户确认 `Approve setup` 后，浏览器自动进入 `/training-ledger/3bba4b6c-0583-4485-9742-352c2e9c0253?feed=strategy`；验证链上 PandaVault/TradingPolicy 对象存在，DB mirror 写入 active vault/policy/account；通过 Feed Strategy 保存并激活 `Trend · RSI` playbook，点击 Start training。 | Training Ledger 显示 `ONLINE`；backend `active_actors=1`，simulation running；market/stale tick 与手动 fresh `DEEP-SUI` tick 均生成 HOLD OrderIntent（最高 `final_score=0.561117`，低于该 Panda `entry_threshold=0.7378`），因此尚无 TradeFact，属于策略/人格阈值导致未执行，不是卡死。 |
| 2026-06-20 | **Training Ledger HOLD 决策可见性修复**：`/training-ledger/:id` 将 `ledger.last_order_intent` 合并进 Decision timeline；`DecisionTimeline` 与 Latest decision 对 `side=HOLD`/`SKIPPED` 显示 `HOLD_OBSERVED` / `Holding`，避免观望决策看起来像卡死；新增 HOLD 渲染与 summary 单测。 | 本地 DB 最新 3 条 OrderIntent 均为 `HOLD/DECIDED`；`corepack pnpm vitest src/components/training/trainingLedgerView.test.ts src/components/training/DecisionTimeline.test.ts --run` 4 passed；`corepack pnpm type-check` 通过。 |
| 2026-06-20 | **Training Ledger HOLD 可见性浏览器复验**：因旧 `localhost:3000` Next 进程处于 `UE` 状态无法释放，改用 `PORT=3001` 启动最新 frontend；用户在 `3001` 重新连接钱包后，执行 Stop → Start → 手动发布 fresh `market:tick:DEEP-SUI`。 | 页面 `Last Action` 显示 `Holding · 0.55`；Decision timeline 从 3 events 增至 4 events，最新行显示 `HOLD_OBSERVED`；复验后已 Stop training，actor 不再运行。 |
| 2026-06-20 | **Training Ledger Paper Trade E2E fixture**：新增 `app.services.e2e_trade_fixture` 与 `backend/scripts/prepare_e2e_trade.py`，用于本地/dev 将指定 Panda 准备成稳定 RSI BUY 执行配置（不暴露生产 API）。 | `pytest tests/test_e2e_trade_fixture.py tests/test_training_hot_path.py tests/test_verdict.py -q` 15 passed；浏览器 `3001` 执行 Start → `/engine/market/tick` fixture tick，DB 出现 `BUY EXECUTED DEEP-SUI final_score=1.202809` 与 1 条 TradeFact；页面 timeline 显示 `PAPER_EXECUTED`，ledger cash 变为 `$9,960` 且显示 `DEEP × 1717.4753`；验收后已 Stop training。 |
| 2026-06-20 | **TradeFact 证据 Drawer 闭环**：训练 API 补齐 `OrderIntent` 的 slippage/reason/proof 字段与 `TradeFact` 的 proof_key/opened_at/closed_at；前端 timeline 点击自动按 `order_intent_id` 绑定 TradeFact；`TradeFactDrawer` 展示执行金额/数量/价格、ledger before/after、阈值、市场 RSI、8-step reasoning。 | 后端 serializer/fixture/hot path 测试 5 passed；前端 TradeFactDrawer/Training view/Timeline 测试 5 passed，`type-check` 通过；浏览器 `3001` 点击 `BUY · DEEP-SUI / PAPER_EXECUTED` 后 drawer 显示 Cash `$10,000 → $9,960`、Position `Flat → DEEP × 1,717.4753`、Final score `1.20`、Entry threshold `0.4875` 与 8 步分数；验收后已 Stop training。 |
| 2026-06-20 | **Review / Skill Memory 前后端闭环**：`prepare_e2e_trade.py` 支持 `--side SELL` 与 contrarian SELL fixture；Review POST 同步处理 review + skill memory 两段 job 并返回最新 skill 数据；`/review` 页面在无 review 时可请求复盘，已有 Skill Memory 时隐藏 “Why not updated?” 并展示 “Panda learned…” 提示。 | 后端 review/fixture/worker 测试 11 passed；前端相关 vitest 5 passed，`type-check` 通过；浏览器 `3001` 用 SELL close tick 生成 `SELL EXECUTED` closed TradeFact，DB 自动写入 `WIN/verified` review + skill memory v1，Review Journal 显示 Realized PnL `+1.2194`、confirmed evidence、Skill memory v1、digest，Evidence 与 Skill diff drawer 均可打开。 |
| 2026-06-20 | **Review Journal 黑金绿视觉优化**：将 Review 页顶部结果、Outcome story、Evidence courtroom、Skill memory、Why-not modal、drawer 内容从浅色后台卡片改为 `ledger-surface`/产品暗色玻璃质感；强化 PnL、hypothesis、entry→exit、Panda learned 层级。 | `corepack pnpm type-check` 通过；相关 frontend vitest 5 passed；浏览器 `3001/review?...` 确认新文案与暗色卡片结构渲染，原白色大卡片已移除。 |
| 2026-06-20 | **P0-1 Safety 对运行中 Agent 即时生效**：`TradeFactWriter.commit_tick()` 在每次执行写入前用当前 DB `TradingPolicy/PandaVault` 重建 fresh policy mirror，避免 `PandaActor` 使用启动时缓存的旧 policy 执行交易。新增回归测试覆盖 stale active mirror + DB paused 时必须拒绝。 | `pytest tests/test_trade_fact_writer_policy_refresh.py -q` 1 passed；相关 `policy_gate/safety/panda_actor/training_hot_path` 测试 17 passed；本地 E2E：actor ONLINE 后将 DB mirror 改为 paused，注入稳定 BUY tick，最新 OrderIntent 为 `BUY REJECTED / POLICY_PAUSED`，未新增 TradeFact；验收后恢复 active policy 并 stop actor，`/health active_actors=0`。 |
| 2026-06-20 | **P0-2 Chain Proof dry-run 状态补齐**：Chain Proof status 增加 `chain_execution.dry_run`；前端 toast、TxDigestCard、Proof details 与 Agent Signer 面板区分 dry-run 与真实 testnet tx，dry-run 不再显示 “Confirmed on testnet” 或 explorer 链接。 | 后端 `test_chain_proof_service/test_chain_execution_worker/test_agent_signer/test_proof_selector` 19 passed；frontend `type-check` 通过；本地临时 `CHAIN_PROOF_ENABLED=true` 且无 signer 私钥时，请求 BUY TradeFact proof 返回 `dry_run=true`、`DRYRUN_...`，浏览器 Chain Proof 页显示 “Dry-run proof recorded / No Sui transaction was submitted”，验收后 backend 已恢复默认配置。 |
| 2026-06-20 | **P0-3 Walrus archive 状态可见化**：`walrus_sync_worker` 在 Walrus 未配置或 publisher 上传失败时写回 Panda `walrus_sync_status=failed` 并返回结构化原因；`queue_dispatcher` 持久化 worker `status=failed` 到 `async_jobs.last_error`；`skill-versions/latest` 增加 `walrus_archive` 聚合状态；Review Skill Memory 卡片展示 archived/pending/unavailable/failed。 | 后端 Walrus/Review 相关测试 22 passed，frontend `type-check` 通过；本地 E2E 入队 `skill_snapshot` archive，当前 testnet publisher `/v1/store` 返回 404，job 落为 `failed`、API 返回 `walrus_archive.status=failed` + 404 错误，Review 页面显示 “Walrus archive failed — skill kept in PostgreSQL”，主 Review/Skill Memory 不受影响。 |
| 2026-06-20 | **P0-4 Strategy 旧入口兼容**：新增 `/strategy/[id]` 兼容页，服务端 redirect 到 `/training-ledger/[id]?feed=strategy`；抽出 `legacyStrategyRedirectPath()` 并补路由单测，确保 U-04 旧验收路径不会 404。 | `corepack pnpm vitest src/lib/ui/routeJump.test.ts --run` 4 passed；`corepack pnpm type-check` 通过；浏览器访问 `localhost:3001/strategy/3bba...0253` 自动落到 `/training-ledger/3bba...0253?feed=strategy`，页面包含 Feed Strategy drawer 内容。 |
| 2026-06-20 | **P1 本地 U-01~U-10 smoke 固化**：新增 `backend/scripts/smoke_u01_u10.py` 自动验证已有本地 Panda 的服务端/Agent 主链路；新增 `dev-docs/local-smoke-u01-u10.md` 覆盖钱包手测步骤、脚本用法、验收信号与失败排查。 | `py_compile` 通过；本地 E2E 使用 Panda `3bba...0253` 跑通：JWT/Panda/Agent Wallet/BUY fixture/SELL close/Review+Skill/Chain Proof status/Walrus failed 可见/Safety `POLICY_PAUSED`/Stop，输出 `SMOKE PASSED`，最终 `/health active_actors=0`。 |
| 2026-06-20 | **P1 Growth 冒烟固化**：新增 `backend/scripts/smoke_growth.py`，验证排行榜、成就、签到的后端 API 与 frontend BFF；`dev-docs/local-smoke-u01-u10.md` 增加 Growth smoke 命令与浏览器 spot checks。 | `py_compile` 通过；`pytest tests/test_leaderboard.py tests/test_achievements.py tests/test_checkin.py -q` 21 passed；frontend `type-check` 通过；本地 E2E 输出 `GROWTH SMOKE PASSED`，浏览器确认 `/leaderboard` 排名、`/achievements` 目录/解锁、`/profile` Daily Check-in `Checked` + `1-day streak` 正常显示。 |
