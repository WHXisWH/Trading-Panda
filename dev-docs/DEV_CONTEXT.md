...
| 2026-06-19 | **K 线 Liquidity 修复** | market-monitor `/health` 增加 `bid_depth`/`ask_depth`/decimals；前端 merge health+pairs 深度并随 Last 价实时重算流动性；0 值改显示 `—`。 | DEEP-SUI 等仅 health 可见的池不再 Liquidity=0；需重启 market-monitor。 |
