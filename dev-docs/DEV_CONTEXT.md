...
| 2026-06-19 | **Training Ledger 手测方案整理** | 基于当前 `/training-ledger/[id]`、market-monitor、WebSocket Hub 与 `docs/local-manual-test-guide.md`，整理了从预飞检查到 start/observe/inspect/stop 的可执行测试路径，并明确了 pair 对齐与 tick freshness 作为首要排障点。 | 未改功能代码、部署事实、环境变量或 API 路径；仅形成可执行手测方案。 |
