...
| 2026-06-19 | **Market tick 频道 canonical + WS 429 + toolbar 指标** | Phase A：`market-monitor` `canonical_market_pair` / `pool_to_pair` → Redis `market:tick:DEEP-SUI`（与 Hub/frontend 对齐）。Phase B：Hub LRU 驱逐最旧连接（替代 HTTP 429）；alarm 清理非 OPEN socket；`useWebSocket` 300ms debounce + 429 退避 5s；`wrangler.toml` `[env.dev]` MAX=8。Phase C：新增 `useToolbarMarketMetrics`（15s REST 1m tail + WS tick，与 chart interval 解耦）。 | 修复 Last/24h 不更新与 dev 429；需重启 market-monitor；`pytest` + websocket vitest + frontend vitest + `tsc` 通过。 |
