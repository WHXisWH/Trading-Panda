...
| 2026-05-10 | **Commit Message 规范化**：将最近两个本地提交改写为 Conventional Commits；新增 `.husky/commit-msg` 和 `scripts/git/validate_commit_msg.py`，强制校验 `type(scope): summary` 格式，允许 Merge/Revert/fixup/squash 系统提交。 | 后续提交若不符合 `build/chore/ci/docs/feat/fix/perf/refactor/revert/style/test(scope): summary` 将被 commit-msg hook 拦截；最近提交已更新为 `chore(tooling): add commit automation` 与 `fix(tooling): parse Gemini changelog JSON` |
