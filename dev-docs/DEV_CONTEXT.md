...
| 经验引擎（5子系统） | 🟡 读路径已接入 | 50% | `ExperienceEngine` load + pattern/mastery 修正接入 Step5；`record_trade` mastery upsert；**缺**：patterns/mistakes/cycles 写回、mistake/cycle 修正汇入、12 表迁移 |
| Merkle Root Worker | 🟡 骨架 | 30% | `compute_merkle_root` 树算法 + actor 每50笔触发骨架；**缺**：leaf 收集（当前传空数组）、`submit_merkle_root` 链上提交、`merkle_roots` 表写入 |
