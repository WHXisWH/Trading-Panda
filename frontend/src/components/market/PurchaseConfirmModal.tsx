    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(400px,95vw)] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-6 shadow-lg animate-scale-in">
          <Dialog.Title className="font-serif text-lg font-bold">
            确认购买
          </Dialog.Title>
          <dl className="mt-4 space-y-2 text-[13px]">
            <div className="flex justify-between">
              <dt className="text-ink-500">熊猫</dt>
              <dd className="font-medium">{listing.name}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-500">价格</dt>
              <dd>{listing.priceSui} SUI</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-500">版税 (2.5%)</dt>
              <dd>{royalty.toFixed(3)} SUI</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-500">Gas</dt>
              <dd>~{gas} SUI</dd>
            </div>
            <div className="flex justify-between border-t pt-2 font-bold">
              <dt>总计</dt>
              <dd className="text-bamboo-500">{total.toFixed(2)} SUI</dd>
            </div>
          </dl>
          <p className="mt-3 text-[11px] text-vermillion">
